...
#include "pch.h"
#include <string>
#import "CSharpHookDLL.tlb"

using namespace CSharpHookDLL;
using namespace std;

void Hook();
void Test();
DWORD WINAPI CallFunc(_In_ LPVOID lpParameter);
...
__declspec(naked)  void Test()
{
	...
	CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)CallFunc, (LPVOID)C_EDX, NULL, NULL);
...
string QrcodeStringSend(long qrcodeAddress)
{
	CoInitializeEx(NULL, NULL);
	CSharpHookDLL::ICSharpHookDLClassPtr CSharpHookDLClassPtr(__uuidof(CSharpHookDLClass));
	_bstr_t result = CSharpHookDLClassPtr->QrcodeStringSend(qrcodeAddress);
	string returnString(result);
	CSharpHookDLClassPtr->Release();
	CoUninitialize();
	return returnString;
}
...
DWORD WINAPI CallFunc(_In_ LPVOID lpParameter)
{
	...
	QrcodeStringSend((long)lpParameter);
	return 0;
}
